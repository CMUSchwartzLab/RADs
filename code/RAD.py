...
def rad_coorddescent(B, M, C, F, max_iter=1000, tol=1e-8):
  ...
  for idx_iter in range(max_iter):
    ...
    if idx_iter >= 500 and abs(previous_error - error) < tol:
      break
    previous_error = error
...
def estimate_clones(B, k, M=None, n_trial=1, verbose=True):
  ...
  for i in tqdm.tqdm(range(n_trial)):
    ...
    C, F, list_err = rad_coorddescent(B, M, C, F)
